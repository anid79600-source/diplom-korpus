...
# Yangi: lemma chastota lug'ati (alohida saqlanadi, freq ga tegmaydi)
if "lemma_freq"   not in st.session_state: st.session_state.lemma_freq   = {}
if "nlp_details"  not in st.session_state: st.session_state.nlp_details  = []
...
def update_frequency(tokens: list):
    ...
    dup_words = 0
    for tok in tokens:
        if not is_valid_token(tok):   # stop-so'z yoki yaroqsiz → o'tkazib yuborish
            continue
...
def process_file(uploaded_file):
    ...
    for lemma, cnt in nlp["lemma_freq"].items():
        st.session_state.lemma_freq[lemma] = (
            st.session_state.lemma_freq.get(lemma, 0) + cnt
        )
    # So'ngi faylning detail ro'yxatini saqlaymiz (tab uchun)
    st.session_state.nlp_details = nlp["details"][:300]   # xotira uchun limit
...
tab1, tab2, tab3, tab4 = st.tabs([
    ...
    "📈 Hisobot",
    "💾 Eksport",
    "🔬 Tokenizatsiya & Lemmatizatsiya",   # ← YANGI
])
...
with tab4:
    if not st.session_state.lemma_freq:
        ...
        # ── Qo'lda matn kiritib sinash ──
        st.subheader("🔬 Matnni sinash (qo'lda kiritish)")
        demo_input = st.text_area(
            "O'zbek tilida matn kiriting:",
            value="Talabalar kutubxonadan kitoblarni o'qimoqdalar.",
            height=80,
        )
        if st.button("Tahlil qilish", type="primary"):
            ...
            c1.metric("Tokenlar",  res["token_count"])
            c2.metric("Gaplar",    res["sent_count"])
            c3.metric("Lemmalar",  len(set(res["lemmas"])))

            # Morfologik tahlil jadvali
            rows = []
            for d in res["details"]:
                rows.append({
                    "Token"     : d["token"],
                    "Lemma"     : d["lemma"],
                    "Morfemalar": " + ".join(d["morphemes"]),
                    "O'zgardimi": "✅" if d["changed"] else "—",
                })
        ...
        # ── Korpus lemma statistikasi ──
        st.subheader("📊 Korpus lemma chastotasi")

        lf = st.session_state.lemma_freq
        total_lemmas = sum(lf.values())

        lc1, lc2 = st.columns(2)
        lc1.metric("Jami lemma (token)", f"{total_lemmas:,}")
        lc2.metric("Noyob lemmalar",     f"{len(lf):,}")

        # Lemma chastota jadvali
        lf_search = st.text_input("🔍 Lemma qidirish", placeholder="kitob")
        lf_rows = [
            {"Lemma": w, "Chastota": c, "Ulush %": f"{c/total_lemmas*100:.2f}%"}
            for w, c in sorted(lf.items(), key=lambda x: -x[1])
            if not lf_search or lf_search.lower() in w
        ]
        lf_df = pd.DataFrame(lf_rows)
        if not lf_df.empty:
            lf_df.index = lf_df.index + 1
            st.dataframe(lf_df, use_container_width=True, height=360)

        st.divider()

        # ── So'ngi fayl tokenizatsiya natijasi ──
        st.subheader("🗂️ So'ngi fayl — tokenizatsiya tafsiloti (birinchi 300 token)")
        if st.session_state.nlp_details:
            detail_rows = []
            for i, d in enumerate(st.session_state.nlp_details, 1):
                detail_rows.append({
                    "#"         : i,
                    "Token"     : d["token"],
                    "Lemma"     : d["lemma"],
            ...
            detail_df = pd.DataFrame(detail_rows).set_index("#")
            st.dataframe(detail_df, use_container_width=True, height=400)

        # Lemma eksport
        st.divider()
        def export_lemma_txt() -> bytes:
            lines = ["lemma\tchastota\tulush%"]
            for w, c in sorted(lf.items(), key=lambda x: -x[1]):
                lines.append(f"{w}\t{c}\t{c/total_lemmas*100:.2f}")
            return "\n".join(lines).encode("utf-8")

        st.download_button(
            label="⬇️ lemmalar.txt — yuklash",
            data=export_lemma_txt(),
            file_name="lemmalar.txt",
            mime="text/plain",
        )
